...
TEST_F(test_x, leftRotation){
    ...
    int len = 4;
    int ranking[] ={3,15,15,15};
    string name[] = {"avengers endgame", "bolt", "cabin boy", "dumbo"};
    int year[] = {2019,2008,1994,2019};
    float rating[] = {8.5,6.5,5.5,6.5};
    
    result = test_LeftRotation(name,ranking, year, rating, len,'b');
    expected = "[GRADER] Adding:(3, avengers endgame, 2019, 8.5)\n[GRADER] Adding:(15, bolt, 2008, 6.5)\n[GRADER] Adding:(15, cabin boy, 1994, 5.5)\n[GRADER] Adding:(15, dumbo, 2019, 6.5)\nRotation successful\nMovies starting with letter: a\n >> avengers endgame 8.5\nMovies starting with letter: b\n >> bolt 6.5\nMovies starting with letter: c\n >> cabin boy 5.5\nMovies starting with letter: d\n >> dumbo 6.5\n\n[GRADER] No leaked memory (Good).\n";
    
    ASSERT_EQ(expected, result);
    add_points_to_grade(5);
    
    
    result = test_LeftRotation(name,ranking, year, rating, len,'a');
    expected = "[GRADER] Adding:(3, avengers endgame, 2019, 8.5)\n[GRADER] Adding:(15, bolt, 2008, 6.5)\n[GRADER] Adding:(15, cabin boy, 1994, 5.5)\n[GRADER] Adding:(15, dumbo, 2019, 6.5)\nRotation successful\nMovies starting with letter: a\n >> avengers endgame 8.5\nMovies starting with letter: b\n >> bolt 6.5\nMovies starting with letter: c\n >> cabin boy 5.5\nMovies starting with letter: d\n >> dumbo 6.5\n\n[GRADER] No leaked memory (Good).\n";
    ...
    int len3= 8;
    int ranking3[]={6, 55, 60, 58, 94,11,40,85};
   string name3[] = {"Pulp Fiction","Avengers", "Bolt","Dumbo","City Lights", "Fight Club", "Memento", "Toy Story"};
    int year3[]={1994,1942,2017,1931,1997,1999, 2000, 2010};
    float rating3[] = {8.9,8.5,8.4,8.5,8.3,8.8,8.5,8,3};
    
    result = test_LeftRotation(name3,ranking3, year3, rating3, len3,'B');

...
string test_LeftRotation(string* name,int* ranking, int* year, float* rating, int len, char r)
{
    MovieTree* tree = new MovieTree();
    testing::internal::CaptureStdout();
    ...
    if(curr)
    {
        ...
        TreeNode* rght = curr->rightChild;
        TreeNode* oldleft = rght->leftChild;
        tree->leftRotation(curr);
        if( curr->rightChild != oldleft || rght->leftChild!= curr)
            cout<< "failed rotation"<<endl;
        else if(oldParent != rght->parent || curr->parent!=rght)
